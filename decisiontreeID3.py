...
#randomize the the dataset and write them back
traindata = loadtxt("train.data",delimiter = ',',usecols = (0,1,2,3),dtype = float)  
trainlabel = loadtxt("train.data",delimiter = ',',usecols = (range(4,5)),dtype = str)
testdata  =loadtxt("test.data",delimiter=',',usecols=(0,1,2,3),dtype=float)
testlabel=loadtxt("test.data",delimiter=',',usecols=(range(4,5)),dtype=str)
...
#print "testlabel"
#print testlabel
for predic,truth in zip(result,testlabel):
    if(predic==truth):
        rightCount+=1
    else:
        wrongCount+=1
...
y_true = np.array(testlabel)
y_pred = np.array(result)
print precision_recall_fscore_support(y_true, y_pred,warn_for=('precision', 'recall', 'f-score'), average='macro')
